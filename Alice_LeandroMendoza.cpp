...
int* climbingLeaderboard(int scores_count, int* scores, int alice_count, int* alice, int result_count[alice_count]) {
	int max=0;
	int cont=0;
	...
	for(int j=1(0); j<alice_count; j++){
		for(int i=0; i<scores_count; i++){
			if(alice[j]>=scores[i]){
				result_count[j] = rank[i];
			}
			if(alice[j]<scores[scores_count - 1]){
				result_count[j] = max + 1;
			}
		}
		}
	return result_count;
